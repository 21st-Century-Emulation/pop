  ...
  def execute: Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
    request.body.asJson match {
      ...
      case Some(json) => json.asOpt[Cpu] match {
        case Some(cpu) => {
          val highByteFuture = ws.url(this.readMemoryApi).withQueryStringParameters(("address", s"${cpu.state.stackPointer}")).get().map(response => response.body.toInt);
          val lowByteFuture = ws.url(this.readMemoryApi).withQueryStringParameters(("address", s"${cpu.state.stackPointer + 1}")).get().map(response => response.body.toInt);
